...
/// <summary>
/// A struct based and disposable utility for tracking elapsed time during operations. Disposable is mostly used to allow greater control of when it should fall out of scope for cleanup, but it also resets the start time before doing so.
/// </summary>
public struct Ticker : IDisposable
{
    ...
    /// Constructor.
    /// </summary>
    /// <param name="start">The option to start the ticker immediately. Defaults to true. If not, it must be started before calling for elapsed ticks.</param>
